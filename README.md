...
npm test             # default unit suite (excludes slow AI simulations)
...
- `npm test` is the default PR unit gate, with a 10-minute CI step budget.
- `npm run test:slow` selects `iso-ai-sweep`, `iso-rebalance`, `iso-debug`,
  `iso-l1d-race` and `battle-ai-sim`. These do **not** run in the required
  default check. The **slow simulations** workflow can be dispatched manually;
  rival-stall failures remain tracked in [#431](https://github.com/rumcan/HexMatch/issues/431).
- `npm run test:all` deliberately includes both lanes.
- Vitest uses at most three workers and stores its cache in the worktree's
  ignored `.cache/vitest/`, not shared `node_modules/.vite`.
- CI also runs the three importable art-tool suites and the CLI guard tests
  on Windows. No shipped art is regenerated by this smoke check.

Agents follow [the playbook's targeted-background-only testing rules](docs/AGENT_PLAYBOOK.md#2-testing-contract-why-and-the-details);
full-suite timing, slow runs and play-testing are the lead's checks.
Maintainers must enable required `test` / `e2e` checks on `main` per
[#210](https://github.com/rumcan/HexMatch/issues/210); a workflow alone cannot
prevent merging a red PR.
