 ...
 *   src/assets/gems/<cargo>.png          the six cargo tokens — ONLY with an
 *                                        explicit `--gems` (see the call site:
 *                                        the board keeps the repo's own hex
 *                                        gems, and this tool must not repaint
 *                                        them by accident)
 *
 * The button/banner art deliberately keeps its painted frame (it reads as an
 * inlaid card on the iron plate). The gems are left alone: HEXmatch is a hex
 * board, so its tokens stay the painted hexagons in `src/assets/gems/`.
 */
...
// GEMS ARE NOT PART OF `everything`: the board's cargo tokens are the repo's
// own HEX gems (128px painted hexagons, pointy-top) and `src/assets/gems/` is
// hand-authored art, not a noir derivative. `--gems` exists for an explicit,
// knowing overwrite; nothing else may reach in there.
if (only("gems")) await gems();
await contact();
await seamCheck();
