...
export function findIsoCorridor(opts?: CorridorOptions): Corridor {
  ...
  // NOTE (PP-02): the Factory-town-adjacency rule lives in `game.placeFactory`
  // via `canPlaceFactory`, so it is enforced on the CLICK. The corridor picker
  // only needs to find a legal, in-view, clickable column; it must not bake the
  // factory rule in here — a town-adjacent factory endpoint is rarely within a
  // 4–7-tile column of an industry-adjacent harvester at the boot camera, so
  // requiring it here would make every gameplay round hang in `setup-factory`.
