 ...
 *  entry instead of hard-coding one cargo. */
const times = (cost: Partial<Record<string, number>>, n: number) =>
  Object.fromEntries(Object.entries(cost).map(([k, v]) => [k, v! * n]));
...
describe("E5 buildability", () => {
  ...
  it("G5: previewDrag with network cannot jump a gap", () => {
    ...
    const rich = { wood: 999, stone: 999, ore: 999 };   // PP-07: road needs wood too
...
describe("W9 the free setup allowance buys road, never rail", () => {
  /** START_PURSE (PP-07 retune): wood + stone for the opening roads, and no
   *  ore at all — the rail gate is unchanged. */
  ...
  it("road still rides the allowance exactly as W1 established", () => {
    ...
    // (PP-07: paid road costs wood + stone per tile)
    const grid2 = flatGrid(), t2 = createTrack();
