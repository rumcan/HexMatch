 ...
 * and with the industry on its NE side the lot opens at the BOTTOM: its SW gate
 * is (hx, hy + 1) — exactly where those fixtures start their road run.
 *
 * True when the lot stands on open ground and nothing else touches its SE or
 * SW side (an industry there would leave no open side, "entrance-blocked").
 */
