...
| The chooser | `ui.ts` `crossPick()`, `.cross-pick` in `styles.css` | Five cargo tiles pop over the board with the angel's moment; each tap spends one of the four units on that cargo, a second tap takes it back (brass ring, an ×N medallion, a "N / 4 spent" counter). A fifth unit is refused until one is freed. The **Bless +4** confirm lights up at exactly four spent; the 8s timer auto-confirms whatever is selected, and the board's own 8s backstop is the second line of defence — a cascade can never hang on an unanswered chooser. #185 later restyled the plate into the house language (shared `.panel` felt over a light board dim, purse-chip tiles, the dialog `.big-btn` confirm, no emoji); the flow is untouched. |
